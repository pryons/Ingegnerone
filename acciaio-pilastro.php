    ...
    public static function prepareDataForRender($project)
    {
        ...
        if (($tipoprofilato == 'TT') OR ($tipoprofilato == 'PC')) {  //tubolari tondi a freddo o caldo
            ...
            if ( $verCL2 <= ($limiteCL1) ) {
                $classesez2 = '1';
            }elseif ( $verCL2 <= ($limiteCL2) ) {
                $classesez2 = '2';
            }elseif ( $verCL2 <= ($limiteCL3) ) {
                $classesez2 = '3';
            }else{
                $classesez2 = '4';
            }
        ...
        }elseif($tipoprofilato == 'UPN'){          // PROFILATO UPN - nessun riferimento normativo - ipotesi da formulare
            $curva = 'c';
            $datajy = $data['jy'];
            $datajt = $data['jt'];
            $datajw = $data['jw'];   
...
//per le verifiche di resistenza e di stabilità devo implementare formulazioni per classe 3 e 4, secondo EC3 e circolare 7/2019.
        ...
        $verStab = ($data["Ned"]*1.05/($chimin*$data["Atot"]*$data['fyk']/10))+($data["Meqx"]*100*1.05/($chiLT*$data['fyk']/10*$wxclasse*(1-($data["Ned"]/$caricoCRx))))+($data["Meqy"]*100*1.05/($data['fyk']/10*$wyclasse*(1-($data["Ned"]/$caricoCRy))));
        if ($verStab > 1){
            $maggminS = '>';
            $classS = 'danger';
            $risS = 'NON';
